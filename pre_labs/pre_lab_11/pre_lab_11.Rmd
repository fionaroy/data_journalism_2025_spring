...
**Answer** The columns all have very long names with spaces, and are listed as <chr> despite containing all numbers (minus the jursidtiction column).
...
**Answer** The numbers 2 through 6 are referring to the specific number of the column in the chart, as in Harris is the second column, Trump is column three to the right of Harris and so on and so forth. 
...
**Answer** The output suggests that there are 54 <li> tags in the HTML source, and does not include visible information about how many <ul> tags are there. However, if there are <li> tags, then there must be at least one or more <ul> tags in which the <li> tags are nested under (most likely one due to the indentation structure).
...
**Answer** The output now shows all specific 54 <li> tags, and their condensed/simplified titles pertaining to what information they each contain. For instance, <li> [1] pertains information regarding Representatives, whereas [21] is regarding Agriculture. (it is just reorganizing the html_elements to display text of the information; simplified)
...
**Answer** The output is a dataframe that has 54 rows and two columns, describing each individual committee, and the specific backslash url html attachment that is specific to the information for each committee (but not the full, complete url for the website).
