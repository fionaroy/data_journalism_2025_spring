...
```{r echo=FALSE}
census_api_key("a8c7acea5866e024b3fcbf50566db8e8e117a1bb", install=TRUE)
```
...
**Answer** Nevada is the state with the largest percentage growth.
...
**Answer** The Maryland county with the highest median household income is Howard County.
...
**Answer** The difference between Calvert and Montgomery County's overlap, and thus the difference is seen as not statistically significant. Although, Calvert County's margin is at least double Montgomery's, which should be taken into consideration, and thus are, at the very least, somewhat different from each other.

Is the difference between Howard and everyone else significant?

**Answer** The difference between Howard County and all the other counties is significant because no other county's bar is anywhere near aligned with Howard, thus equating to their margin of errors being significantly different from each other. 
...
**Answer** (I'm having a hard time reading these numbers, so this is a guess because to me, none of the median incomes appear to have fallen from 2010, to me they all appear to have increased both in estimate and in min and max from 2010 numbers to the numbers in 2020), (also the second question is implying two have changed ['both' implies two], so I'm going to guess). Caroline County and Somerset County on account of their estimates of medium income having the smallest percent change in their income, which (I'm fully guessing here) means that it's relatively close to the margin of error on account of the almost negligent change in income.
...
**Answer** Baltimore City? (I'm really struggling to read these numbers, I think this is the answer...)
...
**Answer** My question is this: How come Cooksville, MD (Howard County) appears to have the largest estimated median household income, but then the next top five (2-6) highest estimated median household incomes all come from Montgomery County? In other words, what is so special about Cooksville, MD?
