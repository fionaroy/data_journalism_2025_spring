...
**Answer** This particular code is using the data provided inside of the primary_24 csv to calculate the percent of votes each individual candidate received on only election day (out of the total amount of votes each candidate received among all voting options).
...
**Answer** The results changed from the previous task due to the additional changes made to the code. By adding a multiply by 100 to the mutating percent command, the percent is put in the correct decimal place to make it easier to read. Additionally, by arranging the data after mutating it in descending order, the data is now structured to show the candidates who received all, or as close as possible, of their votes during this primary exclusively on election day versus those who received votes by other means as well as votes on election day. The previous task only ordered the information alphabetically by county name first, then by candidate name -- not by the actual percents.
...
**Answer** I think that this answer could be more interesting or useful depending on what it is exactly one is looking for within this data. However, I do think that it is interesting to compare both the ascending order and the descending order to see which candidates received all (or almost all)/none (or almost none) or their votes on election day, versus which counties had the highest/lowest turnout on in-person election day voting. In regard specifically to ascending order data, I think that it provides useful information on which counties, and more specifically which candidates, were most unpopular during this election. It would be useful to know as one could write a election follow-up story on why this might be (policy, advertising issues etc.). 
...
**Answer** According to information pulled from stringer.tidyverse.org/reference/case.html, two functions similar to str_to_upper are str_to_title() which will convert data to what is called title case -- only the first letter of every word will be capitalized -- and str_to_sentence() which converts the data to sentence case -- only the first letter of a sentence is capitalized.
...
**Answer** This code's mutate statement is organizing the first six rows of the maryland_winred data to show an additional column, created through the use of case_when inside of a mutate statement, to show which people donated less than 100 dollars, between 100-499 dollars, between 500-1499 dollars, between 500-2999 dollars, and who donated 3,000 or more. As the head function only provides the first six rows of data as a display example, the rows that are present show, with the aid of the new mutate statement, that these donors all donated less than $100.
...
**Answer** The largest category in terms of dollar amount is "Less than $100" as it has the largest amount of money in their total_amount column (more people donated less than 100 dollars total than those who donated between 500-2999 dollars in one go). 
...
**Answer** It is important to make a new variable to hold the result when selecting and not filtering because selecting is different from filtering. Selecting is only meant to extract data from specific columns from the original data source without altering it. Therefore, in order to not make unwanted edits to the original source of data, a new variable needs to be assigned to allow the selected columns/data to be edited or analyzed independently of the original source. 
...
**Answer:** The expenditure_data column, with the assistance of lubridate, now appropriately is characterized as a date column, and the format in which the dates are displayed match the most common global format YYYY-MM-DD. 
...
**Answer** This data in the new 'month' column are showing the total amount of money each month that specific governmental/federal committees are spending in total. This can be understood by looking at how the expenditure_date was mutated to correlate with the new 'month' column, and how it is now grouped by this new 'month' column which is summarized by the total_amount (in descending order largest to smallest charges) of money spent.
