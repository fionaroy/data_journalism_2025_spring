...
**Answer** The problems with this data are that there is no header row, a lot of the rows are riddled with N/As (which is the lack of data, whether by organizational method or general input), and office names are intertwined with candidate names making everything muddled. 
...
**Answer** The problems that remain are that the header-titles are still very confusing and/or non-existent, and the candidate column and party information is still combined which makes the data hard to parse through. 
...
**Answer** What changed is that there is now three distinct columns that seperate the info of candidate name, party affiliation, and the office in which they are associated with. Additionally, there are some specific titles for which numbers/info come from which precinct, but a lot of the columns are still missing proper headings/titles for which the info can be easily understood by. Additionally, there are still some N/As in there on account of missing information.
