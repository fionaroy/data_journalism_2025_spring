...
**Answer** The first line of the warning message (mine is purple) says that "one or more parsing issues, call 'problems()' on your data frame for details, e.g.: dat <- vroom (...)...".
...
**Answer** There are 1,640 problem rows in this dataset. The problem that I think R is describing is there is an ongoing issue with analyzing which candidates have/do not have either provisional or limited votes. 
...
**Answer** When this code is loaded, there is no parsing error/warning message.
...
**Answer**  In the 'district' field, there is <dbl> data. In the 'precinct' field, there is <chr> data. In the 'mail' column, there is <dbl> data. 
...
**Answer**: There are 402,345 results in the mail that are NA. This implies that there are a couple hundred thousand instances in which certain voting precincts/locations received no mail votes for the candidates in this election.
...
**Answer** There are 12 registrations for Yadkin County recorded in the data for January 1900. The first 20 results suggest that there were few, if any, new registrants in Yadkin County from 1900-1967 (except for in the four months in 1966).
...
**Answer** The ones that seem worthy of exploration to me are legacy data (on account of my not knowing what this means: what are the possible implications of there being approx. two thousand?), as well as confirmation not returned and/or returned undeliverable. The last two are of interest because I'm curious as to what makes something necessary of being returned (but not being returned for some reason: what is it?/why?), as well as wanting to know why something is 'undeliverable' (what qualifies something as not being able to be deliverable etc.). The rest i'm not all that interested in becuase they are very straightforward/common. 
...
**Answer** There are 14 rows in this demonstration data set. 
...
**Answer** The type of data in the 'amount' field is <chr> data. 
...
**Answer** The three problems the book chapter says exist in this data set that could prevent one from answering questions are: inconsistent capitalization across multiple column, the zip field mixes five digit ZIP codes and nine digit ZIP codes, the column names have some spaces in them (address one). 
...
**Answer** This function changes the name of "1_linenumber" to "x1_linenumber". 
...
**Answer** The error, when this code is run, says, "error in 'summarise()': in argument: 'total_amount = sum(amount)'. Caused by error in 'sum()': invalid 'type' (character) of argument." I think this means that what the code is trying to 'sum' (add up) -- i.e. 'amount' -- is not a computable variable (it is listed as <chr>). Thus, it cannot run this code. 
...
**Answer** The new data type for 'amount' is now <dbl> (now a summable number not character like before). 
...
**Answer** Each row recorded 2 duplicated counts (four combined). The donor name Derrick Hamilton.
...
**Answer** The new dataframe has 13 rows. 
...
**Answer** What has changed is that the ZIP code column now only includes five digit ZIP codes rather than also include versions where there are nine digits. 
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many misspellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook.

**Answer** There are still two misspellings of Conowingo after running this code. 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many misspellings of Conowingo remain after running this code? Answer below. 

**Answer** There is still one more misspelling of Conowingo remaining after running this code.
