...
**Answer** Okay so this literally does not work on my laptop. I have tried everything, but it will only open PNG's on a PDf reader app and will not open in a browser unless I hack my computer to open it in chrome, but then it just opens as a user/fiona/download file and not the web link you are asking for here. (file:///C:/Users/fiona/Downloads/yaQAT-alsobrooks-election-day-advantage.png)

But! Here is the link that is similar to the map chart, but for this chart: https://datawrapper.dwcdn.net/yaQAT/1/
...
md_gov_county <- md_gov_county |>
  group_by(Fips_Code, County, Cox, Moore, Lashar, Wallace, Harding) |>
  summarise(
    total_votes = sum(Cox, Moore, Lashar, Wallace, Harding)
  ) |>
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. Describe the changes from the previous map.

**Answer** Instead of showing the whole country, including each individual state's counties, with only Maryland and it's counties being lit-up with color, the map (after following instructions) now only shows the state of Maryland and all of its counties color-coded according to percentage of votes for Wes Moore (counties with darker blues being the ones with larger percentages of voters in favor of Moore). 
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} Describe the changes from the previous map.

**Answer** Instead of, when hovering mouse over each county, only the county name and the extended, unrounded, percentage for Moore number showing up, the information is now a lot easier to read: county name, and then below it a four digit percentage nuber of constituents who voted for Moore for Governor in that county (i.e. 00.00%) with a percent sign attached now.
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. Put the URL of your map here.

**Answer** https://datawrapper.dwcdn.net/kZCdX/1/ 
