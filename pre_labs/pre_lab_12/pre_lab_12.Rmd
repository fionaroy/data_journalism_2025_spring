...
**Answer** Dataframe includes, for all of Cardin's press releases, the date, title, url, and full text of each document.
...
**Answer** The unique function gets rid of the duplicated version of the listed words or characters. (it keeps only the first mention of each listed word/character).
...
**Answer** The output of unnest_tokens function breaks down the text column of the releases dataframe into a singular column dataframe where each and every word becomes its own individual row. All lowercase and punctuation is also removed.
...
**Answer** It is not interesting, the most commonly used words are connecting words, filler words; nothing of particular note/not noteworthy. 
...
**Answer** Yes, these results are a lot more interesting as they don't contain only filler/connecting words, but rather descriptive words that would provide insight to Cardin's priorities/what he is most invovled with at this point in time.
...
**Answer** For 2022, it seems the top words are focused around community funding for Baltimore, specifically focusing on health care and governmental action on behalf of the communities health. For 2024, similarly to 2022, the key words seem to be prioritizing bringing to Baltimore health care funding for the community, but this time there is an attached number -- million -- which brings into question the amount of funding, not just a call for it.
...
**Answer** The results of this show that the top two-word phrases for Cardin's 2022 releases are amount included (a focus on money). Next the results show that this is most likely referring to health care, and then which specific cities or county are most likely affected by this call for funds for health care.
...
**Answer** Very similar to 2022's results, except the first results this time are "health care". Other than that, there is still the focus on the "amount awarded" and the specific locations of Maryland most connected to these topics. 
...
**Answer** stem_words seems to have taken off the endings the original list of words (the root). lemmatize_words seems to have just reduced the words to their primary word (getting rid of tense is seems). I think the one that might be more useful for the press releases is lemmatize_words because it can group all types of words that are similar at their core (despite tense differences etc.) which can help with analysis.
...
**Answer** Between the two years, 2022 seems to have prioritized fund(ing), the community, and support as their top three (which isn't very specific until you get further down the list and note that it is most likely about Baltimore). 2024 sees Baltimore at the top of the list, with support, funding, and million right below it painting a pretty clear picture of what is being prioritized (especially since it mentions word like health also which 2022 doesn't). Thus, I think the most useful one is 2024 because it is more specified.  
...
**Answer** This created a new dataframe where each word is listed as either positive or negative in connotation, and the amount of times each word has been recorded. No, the sentiments seem pretty accurate and intuitive if the words are being read at face-value in the most general of ways. But of course close analysis would still have to be used for each word as context means everything, and sometimes something that is listed as a negative like "cancer" could actually be a positive in some cases depending on the information being parsed through.  
