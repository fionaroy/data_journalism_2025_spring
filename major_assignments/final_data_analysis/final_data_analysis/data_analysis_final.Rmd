...
- office of oversight and reform lines 695-728 (mira)
- most newsworthy line 732 (all of us)
- final memo line 737 (all of us)
...
QUESTION ONE: HOW DOES SPENDING VARY AMONG REPRESENTATIVES?
I've got a bar chart of the highest and lowest spending reps, some summary stats, and a normal distribution graphic of the findings.
Findings: Spending varies significantly among representatives, with some spending well over 6 million and some spending under 500K. Most of the spending was somewhere between 4 and 6 million dollars. The highest spender was apparently the governor of Puerto Rico and the lowest spender was a GOP rep from Wisconsin. Normal distribution is skewed right.

```{r}

#READING IN THE DATA, GROUP_BY AND SUMMARIZE

summary_sod <- read_csv("summary.csv")
...
#went into excel, deleted everything that wasn't a rep's office, including leadership offices bc I feel like their budgets might skew things. 

write_csv(grouped_by_org, file = "grouped_by_org.csv")
grouped_by_reps <- read_csv("grouped_by_reps.csv") #new one!
...
```
```{r}
#SORTING BY ORGANIZATION AND WHAT THEY SPENT TOTAL MONEY ON#
 party_caucus |>
...
QUESTION THREE: WHAT DID REP'S SPEND THE MOST MONEY ON, ON AVERAGE FOR 2025 FISCAL YEAR AND QUARTER ONE OF 2025 FISCAL YEAR?
  wanted to find the representatives who spent the most money, and what they spent it on
  have to do both yearly and quarterly b/c this expenditure data is meant to be quarter one of 2025 fiscal year, but also have to already have the 2025 yearly expenditure data ready to be continuously updated because there is left-over/carried-over expenditures that have been ongoing for a while (sometimes multiple years) or haven't been fully processed yet and thus are applicable to the first quarter of the 2025 fiscal year
...
QUESTION FOUR: HOW HAS HOUSE OF REPS SPENDING CHANGED SINCE 2016?
...
It's hard to come to a conclusion on how this stacks up to Brookings's findings because our two datasets (summary and detail) are telling us different things, but personnel compensation was still the highest or second-highest spending category in 2024-2025. That's similar to 2015 spending in the House. 
...
```
--------------------------------------------------------------------------------------------------------
QUESTION FIVE: HOW DID SPENDING FOR THE OFFICE OF OVERSIGHT AND ACCOUNTABILITY/REFORM CHANGE IN RECENT YEARS? 
  Mira noticed some interesting stuff with the Office of Oversight and Accountability and decided to look into it because our data wasn't well equipped to look into our original fifth question. We wanted to see how spending changed for that office over just two years. We started by reading in 2022 data and then joining it with the 2024 transactions data, then doing some calculations for change between the two. 
  This code found a huge change in personnel compensation totals for this office. According to this dataset, it grew by over 3.7 million dollars between 2022 and 2024. That speaks to either the transaction data being messed up for all years, not just 2024/5, or a massive change in spending that might include hiring more people. This could be newsworthy for a couple of reasons: either spending increased dramatically and we'd have to find out why, or the House has been keeping faulty and incomplete transaction data for multiple years. Whichever it is, it would be worth digging into. 
...
```{r}
#looking for differences in expenditures between 2022 and 2024/5##chat was somewhat consulted for this#
detail_22_sod <- read_csv("transactions_2022.csv") |>
  clean_names()
...
accountability_24 <- transactions %>% 
  filter(organization == "2024 COMMITTEE ON OVERSIGHT AND ACCOUNTABILITY") %>% 
  group_by(organization, sort_subtotal_description, description, amount, fisca_or_legislative_year) %>% 
  summarize(count = n(), total_spent = sum(amount, na.rm = TRUE), .groups = "drop") 

#joining these two year-based expenditure data together and calculating change in spending#
joined <- accountability_22 |>
  left_join(accountability_24, join_by("description" =="description")) %>% 
...
#personnel compensation change is on the second line of the dataset. Personnel compensation grew  $3766143.50 between 2022 and 2024 for oversight and accountability office.
...
-----------------------------------------MOST INTERESTING FINDING?-------------------------------------
  ...
  Out of all of our cleaning and data analysis of House of Representative Expenditure Data so far for this project, the most newsworthy findings thus far have been our findings for question four: Has the House of Representatives spending changed since the 2016 Brookings article? What stuck out to us the most in these findings was not only the fact that it covered roughly the span of ten years worth of expenditure data --in the sense that we could track ten years worth of changed (or not changed) priorities in regard to federal spendings-- but also what the federal expenditure data is specifically continuously (or not continuously) lacking in regard to their data record keeping abilities. So far, among all of our questions we've noticed a strong continued trend of poor record keeping which has severely limited our ability to properly analyze the House of Representative expenditures. However, this is also a limited boon as it has increased the amount of questions we originally had (as in the more we dig in, the more questions we have about why the data is missing). Pertaining to this question, what we found follows the trend we've noticed throughout all the data in that the most notable differences in expenditure data over the years is in regard to personal compensation, 'other' (due to lack of provided data), and personal office spending. We found that in each of these categories, spending increased by large amounts from 2015 to 2024/5. We’d like to know more about why spending increased, whether it outpaced inflation, and why different spending categories made up different percentages in 2015 versus 2024/5. 


-----------------------------------FINAL MEMO-----------------------------------------------------------

  There’s a massive difference in spending between the U.S. Representatives equating to millions of dollars. Specifically, personnel compensation is vastly different for the staff of each representative included in the data. This is newsworthy because of the spending disparities and what they mean for constituents. Each representative is paying their staff different amounts for their quarterly and yearly salaries, according to Fiona’s analysis of summary data. And while federal regulations set minimum and maximum salaries, there is enough variation to be of significant interest. Are two people doing the same job, but receiving different salaries because they work in different states? What do state regulations have to do with congressional spending? Where is this money coming from? 
  Personnel compensation appeared consistently during analysis. We looked at how spending varied and found that most offices spent between 4 - 6 million (already a wide gap), but other offices were spending a fraction of that. We also looked at how spending varied among the Democratic and Republican caucuses, and how House spending changed between 2015-2024/5. We also found that personnel compensation increased for the Office of Oversight and Reform/Accountability between 2022 - 2024. We’d look into Oversight spending, and whether the change in spending between 2015-2024/5 correlated with inflation.
  These findings are new, but we were working with both current and old data. There is potential for a local angle by examining Maryland expenditures, but we are more focused on a national comparison to answer our questions. 
For pros and cons, the summary data was well done. The data included representatives, caucuses, and other offices and employees. The structure made it easy to analyze using the tools we learned in class. However, the more detailed transaction data was extremely flawed, with significant missing data, and we are unsure if our analysis results are correct. The flaws in the data did bring up more questions to explore —why was it so incomplete? What does that mean for government record keeping?— but our analysis is limited without a complete dataset. 
  Firstly, we’d seek verification from whoever manages digital records for the House, and ask about the missing data. If they have other records to fill in those gaps, we’d want them. We’d also verify our findings with them. Next, we’d speak to individual representatives about their spending, especially those spending the most/least. We’d ask about their staff salaries, why each state appears to be spending differently on staff members of the same positions across the country, and what that might mean for representative priorities and/or state priorities. Also, we’d ask where their money is coming from for personnel compensation.
  To begin we would clarify analysis verification: reaching out to House comms to see who we can talk to about the data discrepancies. Next, we’d reach out to the top five and bottom five spenders in the House. We’d also look for a political science data analyst for an academic perspective. We’d travel to wherever these spending records are physically stored, and to the offices of representatives if we’re able. 
