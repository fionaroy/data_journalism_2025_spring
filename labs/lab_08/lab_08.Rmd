---
title: "lab_08"
author: "derek willis"
date: "2024-10-21"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

In this lab we'll be looking at Maryland absentee ballot requests in the 2024 election and making several charts based on that data. The state of Maryland reports the number of ballots sent to voters by counties and the number received by counties from voters. We'll use both types of information.

## Load libraries and establish settings

**Task** Create a codeblock and load appropriate packages and settings for this lab. We'll be making some charts and cleaning up data.

```{r}
# Turn off scientific notation
options(scipen=999)

# Load libraries
library(tidyverse)
library(janitor)

##b/c of mira help##
install.packages("ggrepel")
library(ggrepel)
```

## Load data

**Task** Create a codeblock and load the following data from the data folder:

-   Maryland absentee ballots sent and returned - `md_absentee_ballots.csv`
-   Maryland absentee ballots sent and returned by date - `md_absentee_ballots_date.csv`

You can clean up column names to make them easier to work with if you like, and should ensure that any date columns are actually date formatted.

```{r}
# load the data
md_ballots <- read_csv("data/md_absentee_ballots.csv")

md_dates <- read_csv("data/md_absentee_ballots_date.csv")


##cleaning column names so no space##
md_dates <- md_dates |>
  clean_names()

md_ballots <- md_ballots |>
  clean_names()
```

## Questions

**Q1.** Which county has the highest percentage of total absentee ballots returned of total ballots sent? Make a bar chart of the top 10 counties. Your bar chart must have:

-   A clear title that states the main idea/finding
-   Good labels for the x & y axis and a caption for the source, which is the Maryland State Board of Elections
-   Readable bars - the values shouldn't be overlapping

**A1.** The county that has the highest percentage of total absentee ballots returned of total ballots sent is Worchester County, 62.59%.

```{r}

##finding top ten##
md_ballots <- md_ballots |> 
  mutate(absentee_diff = total_received/total_sent*100)

maryland_total_pct_returned <- md_ballots |>
  arrange(desc(absentee_diff)) |>
  select(county_name, absentee_diff) |> 
  head(10)

maryland_total_pct_returned

```

```{r}

##making the total returned barchart##
maryland_total_pct_returned |>
  ggplot() +
  geom_bar(aes(x=reorder(county_name,absentee_diff), weight=absentee_diff)) +
  coord_flip() + 
  theme_minimal() +
  labs(
    title="Maryland Counties: Highest Percent Returned Total Absentee Ballots",
    x = "Maryland County",
    y = "Percent Returned",
    caption = "Source: Maryland State Board of Elections" 
    )


```


**Q2.** What are the top 10 counties with the lowest percentage of ballots returned by Democratic voters, and what is the difference between the percentage returned for Democrats and Republicans in those counties? You MUST do the following things when answering this question:

-   Make a codeblock below to write your code in.
-   Calculate the percentage of ballots returned by both Democrats and Republicans.
-   Arrange the dataframe to show the counties with the smallest percentage of returned ballots from Democrats first.
-   Use ggplot to make a horizontal bar chart that shows just the first 10 counties, with the length of the bar determined by the county's percentage of ballots returned by Democrats. Give it an appropriate title, source, and x and y axis titles.

**A2.** The answers are in the code down below, along with the Chart depicting the information pertaining to Democrat Ballots. In terms of difference between dem and rep's, dems have a higher absentee ballot return rate than rep's.


```{r}

##finding bottom 10 dem##
md_ballots <- md_ballots |> 
  mutate(dem_diff = dem_received/dem_sent*100)

maryland_dem_pct_returned <- md_ballots |>
  arrange(desc(dem_diff)) |>
  select(county_name, dem_diff) |> 
  arrange(dem_diff) |>
  head(10)

maryland_dem_pct_returned

```

```{r}

##finding bottom 10 rep##
md_ballots <- md_ballots |> 
  mutate(rep_diff = rep_received/rep_sent*100)

maryland_rep_pct_returned <- md_ballots |>
  arrange(desc(rep_diff)) |>
  select(county_name, rep_diff) |> 
  arrange(rep_diff) |>
  head(10)

maryland_rep_pct_returned

```

```{r}

maryland_pctchange_combined <- maryland_dem_pct_returned |> inner_join(maryland_rep_pct_returned)

maryland_pctchange_combined |>
  mutate(difference = dem_diff - rep_diff)


```


```{r}

##making the dem barchart##
maryland_dem_pct_returned |>
  ggplot() +
  geom_bar(aes(x=reorder(county_name,dem_diff), weight=dem_diff)) +
  coord_flip() + 
  theme_minimal() +
  labs(
    title="Maryland Counties: Lowest Percentage Returned Democrat Ballots",
    x = "Maryland County",
    y = "Percent Democrat Ballots Returned",
    caption = "Source: Maryland State Board of Elections" 
    )

```


**Q3** Let's make a line chart using the second set of absentee ballot data - this one tracking returns by date. We want to see received ballots for Frederick County for Democrats, Republicans and Others, so you'll make a chart with three lines, adding color to each one (you can choose, but choose sensibly). Give the chart a title and good labels. What's the story that this line chart tells? Compare and contrast the three lines. Does it match what you know about absentee voting habits?

**A3** The story that this line chart is telling is that in Frederick County there is a significantly larger number of Democrat ballots being received than either Republican or Other. This does match what I know about absentee voting habits for Maryland, and in particular what I have learned this semester in this class about Frederick County (that there is a stronger dem presence than rep or other).

```{r}

md_dates |>
  filter(county_name == "FREDERICK") |>
  ggplot() +
  geom_line(aes(x=date, y=dem_received), color = "#00008B") +
  geom_line(aes(x=date, y=rep_received), color = "#8B0000") +
  geom_line(aes(x=date, y=oth_received), color = "#FF69B4") + 
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d") +
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)
  ) +
  labs(
    title="Frederick Counties: Amount of Received Returned Ballots by Date",
    x = "Date",
    y = "Return Ballots Received",
    caption = "Source: Maryland State Board of Elections" 
    )

```

**Q4.** A scatterplot is a type of chart that helps us see relationships between two variables. One variable goes on the x axis, the other on the y axis. For each row/observation in our data, a scatterplot puts a circle (or a "point") where the two variables intersect on a grid.

Statisticians use scatterplots to show graphically whether one variable is correlated -- related, in a statistical sense -- with another variable. A classic example is the [relationship between ice cream sales and temperature](https://www.mathsisfun.com/data/scatter-xy-plots.html). The scatterplot below -- press play to load the image -- shows that relationship, that an increase in temperature is associated with an increase in ice cream sales. When it's 12C, sales are 200 dollars, and when it's hotter, 25C, sales are 600 dollars.

```{r}
knitr::include_graphics("https://www.mathsisfun.com/data/images/scatter-ice-cream1.svg")
```

We're going to use a scatterplot to get a visual sense of two key variables: ballots sent and ballots returned. We want to plot both of those for each county in Maryland. IMPORTANT: you need to choose whether you use total ballots or one of the parties.

Follow these steps:

1. Using the first dataframe (the one without dates), make a scatterplot showing the number of ballots sent on one axis and the number received on the other axis. The pre_lab didn't show you how to do this, so look it up! https://ggplot2.tidyverse.org/reference/geom_point.html?q=geom_point#aesthetics is a good start.
2. Give it an appropriate title, source, and x and y axis titles.
3. Try to add a label for each point that shows the name of the county using geom_text(). You might need to find alternative ways to do this to make them readable.
4. Add a line of fit to the plot using geom_smooth (https://ggplot2.tidyverse.org/reference/geom_smooth.html#ref-examples). Specify the method as lm.
5. In the answer space below, describe what you see. In a general sense, how would you report out the main point(s) of this chart? Are there counties that seem to be over or under-performing in relation to the overall trend?

**A4.** I see a scatterplot of Maryland counties total sent and received ballots. I would report out the main points of the chart by finding the counties that are significant outlines from the others (i.e. the ones that are clustered very close together on account of similar ballot sent and received numbers) and report on why these counties are outlines. By reading the chart, for example, it appears that counties Caroline, Kent, Garrett, Somerset and Dorchester are under performing due to very low numbers of sent and received ballots. Thus, I would look at why this might be, and how they might be trying to fix it. Same thing with the over performing counties: Montgomery, Prince George's and Baltimore County.

```{r}
##got help from mira for some of it [who got some help from chat to fix up scatterplot]##

scatterplot_totalballots <- md_ballots |>
  select(county_name, total_sent, total_received)

scatterplot_totalballots |>
  ggplot() +
  geom_point(aes(x=total_sent, y=total_received), color = "#FFA500", size = 4) +
  geom_label_repel(aes(x=total_sent, y=total_received, label = county_name), 
                   size = 4, 
                   box.padding = 0.35, 
                   point.padding = 3,
                   segment.color = "#000000",
                   segment.size = 0.5,
                   max.overlaps = 60,
                   fontface = "bold") +
  labs(
    title = "Maryland Counties Absentee Ballots Recieved by Total Sent",
    x = "Total Ballots Sent",
    y = "Total Ballots Received",
    caption = "Source: Maryland State Board of Elections") +
  geom_smooth(aes(x=total_sent, y=total_received), method = "lm", color = "purple", size = 1)
  
```
