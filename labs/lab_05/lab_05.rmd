...
harris_donations_combined <- bind_rows(list(august_harris, july_harris))
...
#harris_combined_and_new_voters <- 
  
harris_donations_combined <- harris_donations_combined |>
...
A1. The columns that were selected for this join were done together as a class, but, were selected because they were the one's that were similar between the two data sets. There is always a chance for false positives, however, this is more unlikely becuase we have joined as many as possible to offset this by including specific columns such as first name, last name, address and city.
...
A2. The amount of donations that have come from people who registered this year is 326, 77 of which have been since July 20. Brandyn Best gave the most amount of donations since July 20 with 4 donations. 
...
harris_donations_combined |>
  group_by(CountyRegistrationDate, last_name, first_name, amount) |>
  summarise(count=n()) |>
  filter(CountyRegistrationDate >= mdy("07/20/2024")) |>
  arrange(desc(count))
...
A3. Harris obtained 32 donations from non-democrats. The gender breakdown can be found down below.
...
A4: I would like to know how many of these registered voters lived in Charles County. (18)
